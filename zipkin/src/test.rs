...
use {Report, Span, Tracer, Endpoint};
...
fn detach() {
    ...
    let tracer = Tracer::builder()
        ...
        .build(Endpoint::builder().build("test"));

    let parent = tracer.new_trace("parent");
    let parent_trace = parent.context().trace_id();
    let parent_id = parent.context().span_id();

    let mut detached = tracer.next_span("detached");
    let detached_id = detached.context().span_id();
    detached.detach();

    let child2 = tracer.next_span("child2");
